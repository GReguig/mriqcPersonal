...
class BaseSearchCV(BaseEstimator, MetaEstimatorMixin, metaclass=ABCMeta):
    ...
    def __init__(self, estimator, scoring=None, n_jobs=None, iid='warn',
                 ...
                 error_score='raise-deprecating', return_train_score=True, to_evaluate=None):
        ...
        **fit_params : dict of string -> object
        ...
        with parallel:
            ...
            def evaluate_candidates(candidate_params):
                ...
                #if self.verbose > 0:
                LOG.info("Fitting {0} folds for each of {1} candidates,"
    ...
    def _format_results(self, candidate_params, scorers, n_splits, out):
        ...
        if self.return_train_score and self.to_evaluate:
            (train_score_dicts, test_score_dicts, test_sample_counts, fit_time,
             score_time, eval_score) = zip(*out)

        elif self.to_evaluate:
            (test_score_dicts, test_sample_counts, fit_time,
             score_time, eval_scores) = zip(*out)

        elif self.return_train_score:
            (train_score_dicts, test_score_dicts, test_sample_counts, fit_time,
             score_time) = zip(*out)
        ...
        if self.to_evaluate:
            eval_scores = _aggregate_score_dicts(eval_scores)
        results = {}
        ...
        for key_dataset_score, ds_score in eval_scores.items():
            _store(key_dataset_score, ds_score, splits=True)
...
class GridSearchCV(BaseSearchCV):
    ...
    def __init__(self, estimator, param_grid, scoring=None,
                 ...
                 return_train_score=False, to_evaluate=None):
        super().__init__(
            estimator=estimator, scoring=scoring,
            n_jobs=n_jobs, iid=iid, refit=refit, cv=cv, verbose=verbose,
            pre_dispatch=pre_dispatch, error_score=error_score,
            return_train_score=return_train_score, to_evaluate=to_evaluate)
        self.param_grid = param_grid
