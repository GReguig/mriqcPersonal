    ...
    def transform(self, X, y=None):
        ...
        for gid, batch in enumerate(glist):
            if batch in self._scalers:
                ...
                print("Version modified in mriqc/classifier/sklearn/preprocessing line160")
                """
...
class ColumnsScaler(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, with_centering=True, with_scaling=True, columns=None):
        #super(ColumnsScaler, self).__init__()
        self.with_centering = with_centering
        self.with_scaling = with_scaling
        self._scaler = None
        self.columns = columns
    ...
    def _numeric_cols(self, X):
        ...
        if "subject_id" in numcols:
            numcols.remove("subject_id")
    ...
    def fit(self, X, y=None):
        """
        self._scaler = StandardScaler(with_mean=self.with_centering, 
                                      with_std=self.with_scaling)
        """
        self._scaler = RobustScaler(with_centering=self.with_centering,
    ...
    def transform(self, X, y=None):
        ...
        scaled_x = pd.DataFrame(self._scaler.transform(
            X[columns]), columns=columns, index=X.index)
        unscaled_x = X.iloc[:, ~X.columns.isin(columns)]
        result = pd.concat([unscaled_x, scaled_x], axis=1)[col_order]
...
class PandasAdaptor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if "subject_id" in self.columns:
            self.columns.remove("subject_id")
        return self
    ...
    def fit(self, X, y, n_jobs=1):
        ...
        while noise_flag:
            ...
            if np.all(importances[0:-1] > k * importances[-1]):
                ...
                print("All features (%d) are better than noise", len(idx_keep) -1)
                ...
                print("No feature better than noise")
