...
LOG = logging.getLogger("mri_classif")


def cross_val_score(estimator, X, y=None, groups=None, scoring=None, cv=None,
                    n_jobs=1, verbose=0, fit_params=None,
                    pre_dispatch='2*n_jobs', datasets=None):
    """
    ...
    scores = parallel(delayed(_fit_and_score)(clone(estimator), X, y, scorer,
                                              ...
                                              fit_params, to_evaluate=datasets)
...
def _fit_and_score(estimator, X, y, scorer, train, test, verbose,
                   ...
                   return_times=False, error_score='raise', to_evaluate=None):
    """
    ...
    try:
        ...
        if error_score == 'raise':
            ...
            warnings.warn("Classifier fit failed. The score on this train-test"
                          ...
                          "Details: \n{} \n model: {}".format(error_score, e, estimator), FitFailedWarning)
        else:
    ...
    if to_evaluate:
        ...
        for ds_name, ds_vals in to_evaluate.items():
            for s in scorer:
                res_evaluation[ds_name + "_" + s] = _score(estimator, ds_vals["x"], ds_vals["y"], s)
...
def _score(estimator, X_test, y_test, scorer):
    ...
    scorer = check_scoring(estimator=estimator, scoring=scorer)
    if y_test is None:
